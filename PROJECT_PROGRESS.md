...
| 2026-09-02 | **v1.3.0 - Probhat layout Sprint 1 (100% correct)** | Separate Rust profile `core/src/layouts/probhat.rs` (HashMap<KeyCode,Glyph> with primary/secondary, HASANTA U+09CD on '/'), engine `load_layout` now sources Probhat map from Rust profile (guaranteed separation, no mixing). Verified `probhat.json` (core + assets) identical (31 base keys). Added `core/tests/probhat_layout_tests.rs` (18 tests: all primaries, shift secondaries, vowel signs কা/কি/কে, halant conjuncts ক্চ/ক্ষ/ত্র, words কথা/দেশ/বাংলা/মানুষ/বাংলাদেশ, backspace grapheme). UI already QWERTY Bengali glyphs via LayoutManager + logical IDs + combinedClickable long-press. All 205 tests pass (91 lib inc. probhat + 18 probhat). versionCode 8. |
| 2026-09-02 | **v1.3.1 - Fix OTA wrong package validation** | Fixed "Update file is invalid (wrong package)" on Android 16: `isValidApkForThisApp` now uses `PackageInfoFlags.of(0)` on API 33+ (fixes deprecated int-flag returning null) with logging and permissive fallback (returns true to let PackageInstaller decide). `installApk` now calls `params.setAppPackageName(context.packageName)` ("com.keybroad") to match `applicationId` and `update_config.json` `package_name`. Updated `getApkSignature` for Tiramisu. All 205 Rust tests still pass, assembleDebug success. versionCode 9. |
| 2026-09-02 | **v1.3.2 - Full diagnosis: typing + OTA** | Reproduced typing: Probhat `k,shift+f,a` → `কথা` correct (prior test used wrong sequence `k,a,shift+f,a` → `কাথা`), verified `বাংলা`/`ক্চ`/`ক্ষ`/`ತ್ರ`, JNI state preserved across calls, full text via `engine.get_text()`. Verified OTA: `aapt dump badging` confirms `package: name='com.keybroad'` for both debug/release; `SessionParams.setAppPackageName("com.keybroad")` set, `isValidApk` now API36-safe. Added `core/tests/typing_repro_test.rs` (5 tests: কথা, বাংলা, ক্চ, state, uppercase task-spec). All 210 tests pass (91 lib + 18 probhat + 5 repro), assembleDebug success. |
| 2026-09-02 | **v1.4.0 - Bootstrap + typing UI verified** | Verified stable signing: `assembleRelease` signed `d715e5bf6f759dd778b9ab8d8406a84058d854e7e0fafd461739d9bea99b87c8` for both debug/release, `aapt dump badging` `package: name='com.keybroad' versionCode='11' versionName='1.4.0'`. Typing UI: ViewModel sends logical IDs (`k`→107) + ` _text.value = result` (engine single source), View Probhat glyphs via `probhat.json` QWERTY. No UI change needed — verified via `jni_typing_tests` (k→ক, h→খ, k,a→কা) and `probhat_layout_tests` (18). Bootstrap APK ready for one-time manual install; future OTA via `PackageInstaller` with stable key will be seamless. |
| 2026-09-02 | **v1.5.0 - Gboard alternative (English + Avro Bangla)** | Radical redesign: removed Jatiya/Probhat/Unijoy switcher, Gboard-style light theme (white keys, gray bg, rounded 6dp, 1dp shadow), 3-row QWERTY + bottom row `?123`/`🌐`/`Space(EN/বাংলা)`/`↵`. Dual mode `isEnglishMode` boolean: Globe toggles `Phonetic`↔`English` via `engine.switchLayout`. Bangla Avro: `ami`→`আমি` via `PhoneticEngine` + Bengali `getSuggestions`; English: direct `englishWords` filtered suggestions (`hel`→`hello`). LayoutManager now `getRomanKeys()` only, `MainActivity` no longer shows `LayoutSwitcher`. All 210 tests pass, assembleDebug success. |
